...
for item in scrape_list["scrape_list"]:
  ...
  elif item['type']  == "shiro":
    scraper = ShiroShiroScraper(failed_logger, success_logger, info_logger, will_hide, False)
    result = scraper.start()
    scraper.close()
    if result['is_success'] is True:
      summary_logger.log('SHIRO SHIRO SUCCESS')
      ...
      summary_logger.log('SHIRO SHIRO FAILED')
